const translations = {
  ko: {
    title: "Clinisch & Co. | 부티크 임상 서비스",
    nav_about: "소개",
    nav_services: "서비스",
    nav_approach: "어프로치",
    nav_contact: "문의하기",
    lang_toggle_aria: "언어 전환",
    hero_eyebrow: "부티크 임상 서비스 기업",
    hero_title: "임상 리더십과 데이터 사이언스 & AI가 만나는 곳",
    hero_lead:
      "경험 많은 임상 리더십을 기반으로 실행력과 데이터 기반 인사이트를 결합해 " +
      "부티크 임상시험을 민첩하고 효율적으로 지원합니다.",
    hero_cta_primary: "미팅 요청",
    hero_cta_secondary: "서비스 보기",
    hero_badge1: "임상 리더십",
    hero_badge2: "린(Lean) 운영",
    hero_badge3: "AI 기반 인사이트",
    panel_title: "회사 한 줄 소개",
    panel_text:
      "임상 리더십과 실행 경험을 중심으로, 데이터 사이언스와 AI를 접목한 " +
      "부티크 임상 서비스 기업입니다.",
    panel_tile1_title: "APAC · EU · US",
    panel_tile1_text: "글로벌 리전 실행 경험.",
    panel_tile2_title: "즉시 가용성",
    panel_tile2_text: "필요 시 빠른 투입 가능.",
    panel_tile3_title: "유연한 계약",
    panel_tile3_text: "모듈형 확장 가능한 운영 모델.",
    about_eyebrow: "회사 소개",
    about_title: "시니어가 직접 이끄는 실행 중심 임상 지원",
    about_body:
      "다년간의 임상시험 전주기를 거쳐 다양한 환경과 현장 중심의 임상 지원을 " +
      "제공하는 부티크 임상 서비스 기업입니다. 풍부한 경험을 갖춘 시니어 임상 " +
      "리더가 프로젝트를 직접 이끌며 데이터 사이언스와 AI 기반 도구를 활용해 " +
      "의사결정을 명확히 하고 일정 최적화와 운영 리스크 최소화를 구현합니다.",
    about_card1_title: "차별화 포인트",
    about_card1_text:
      "전통적인 CRO 모델과 달리, 소수 정예의 린(Lean) 운영과 직접적인 " +
      "커뮤니케이션으로 불필요한 복잡성을 줄이고 실행 중심의 지원을 제공합니다.",
    about_card2_title: "미션 & 비전",
    about_card2_text:
      "시니어 리더십이 주도하는 임상 서비스를 제공하고, 대형 CRO 대비 운영 " +
      "격차를 메우는 부티크 파트너가 되는 것을 목표로 합니다.",
    slides_eyebrow: "슬라이드 하이라이트",
    slides_title: "핵심 메시지 스냅샷",
    slides_body: "미팅 핵심 메시지를 요약해 배치했습니다.",
    slide1_title: "Clinisch & Co.",
    slide1_text: "임상 리더십과 데이터 사이언스 & AI가 만나는 곳",
    slide2_title: "Why Clinisch & Co.",
    slide2_text: "시니어 주도, 유연한 모델, 기술 기반 가시성.",
    slide3_title: "How We Work",
    slide3_text: "명확한 커뮤니케이션과 빠른 실행.",
    services_eyebrow: "서비스",
    services_title: "임상시험 전주기 맞춤 지원",
    services_body: "필요와 단계에 맞춰 유연하게 설계합니다.",
    service1_title: "임상 프로젝트 관리",
    service1_item1:
      "기획부터 결과 도출까지 시니어 주도의 임상 프로젝트 관리",
    service1_item2: "임상시험 계획 수립 및 일정·마일스톤 관리",
    service1_item3: "내부 팀 및 벤더 조율",
    service1_item4: "리스크 선제 식별 및 이슈 해결",
    service1_outcome:
      "기대 효과: 명확한 책임 구조 · 예측 가능한 일정 · 이해관계자 정렬",
    service1_icon: "임상 프로젝트 관리 아이콘",
    service2_title: "스타트업 및 시험 실행",
    service2_item1: "규제 및 IRB/EC 제출 지원",
    service2_item2: "시험 개시 및 준비 지원",
    service2_item3: "임상시험 실행 및 운영 지원",
    service2_item4: "주요 사이트 중심 코디네이션",
    service2_outcome: "기대 효과: 빠른 시험 개시 · 안정적 운영 · 리스크 최소화",
    service2_icon: "시험 실행 아이콘",
    service3_title: "스터디 구조 개선 및 운영 감독",
    service3_item1: "지연·중단 위기 임상시험 정상화",
    service3_item2: "임시 임상/운영 리더십 제공",
    service3_item3: "원인 분석 및 교정·예방 조치 수립",
    service3_item4: "지속적 운영 관리 및 품질 개선",
    service3_outcome: "기대 효과: 통제 회복 · 일정 정상화 · 신뢰 회복",
    service3_icon: "스터디 구조 개선 아이콘",
    service4_title: "임상 시스템 및 데이터 활성화",
    service4_item1: "임상 시스템 맵 설계 및 최적화",
    service4_item2: "데이터 기반 운영 대시보드 구축",
    service4_item3: "효율성과 품질을 높이는 AI 도구 적용",
    service4_item4: "프로세스 자동화 및 데이터 통합 지원",
    service4_outcome: "기대 효과: 가시성 향상 · 신속한 의사결정 · 운영 효율화",
    service4_icon: "임상 시스템 및 데이터 활성화 아이콘",
    approach_eyebrow: "어프로치",
    approach_title: "기술은 결정력을, 경험은 방향을 만듭니다.",
    approach_card1_title: "임상 리더십",
    approach_card1_text: "시니어 임상 리더가 전략과 실행을 직접 주도합니다.",
    approach_card2_title: "데이터 기반 운영",
    approach_card2_text: "운영 인사이트를 데이터로 빠르고 명확하게 제공합니다.",
    approach_card3_title: "AI 기반 실행",
    approach_card3_text: "AI 도구로 효율성과 리스크 관리를 강화합니다.",
    work_eyebrow: "일하는 방식",
    work_title: "부티크 임상 조직을 위한 유연한 협업",
    work_card1_title: "협업 옵션",
    work_card1_text: "주 단위 지원 · FTE 기반 지원 · 프로젝트 단위 패키지",
    work_card2_title: "확장 가능한 지원",
    work_card2_text: "임상시험 단계와 필요에 따라 유연하게 확장·축소합니다.",
    clients_eyebrow: "대상 고객",
    clients_title: "속도와 품질의 실행을 중시하는 파트너",
    client_tag1: "바이오·의료기기 스타트업",
    client_tag2: "제약사 및 중견 규모 기업",
    client_tag3: "내부 임상 조직 운영이 필요한 파트너",
    client_tag4: "특정 영역의 임상 리더십이 필요한 조직",
    why_eyebrow: "왜 Clinisch",
    why_title: "불필요한 오버헤드 없이, 시니어 주도 실행",
    why_card1: "시니어 중심의 부티크 임상 서비스 모델",
    why_card2: "실행에 집중하는 운영 지원",
    why_card3: "임상 운영과 글로벌 경험을 결합한 데이터 사이언스 & AI",
    why_card4: "민첩하고 린한 운영 구조",
    why_card5: "명확한 커뮤니케이션과 빠른 속도",
    cta_eyebrow: "콜 투 액션",
    cta_title: "대형 CRO는 부담스럽고, 유연한 임상 지원이 필요하신가요?",
    cta_body: "귀사의 임상시험에 어떤 방식이 적합할지 함께 논의해 보세요.",
    cta_role: "구연의 | 대표 & 매니징 멤버",
    label_email: "이메일",
    label_support: "지원",
    label_tel: "전화",
    label_mobile: "모바일",
    cta_location: "대한민국 인천",
    footer_text: "임상 리더십과 데이터 사이언스 & AI가 만나는 곳"
  },
  en: {
    title: "Clinisch & Co. | Boutique Clinical Services",
    nav_about: "About",
    nav_services: "Services",
    nav_approach: "Approach",
    nav_contact: "Contact",
    lang_toggle_aria: "Language toggle",
    hero_eyebrow: "A Boutique Clinical Services Company",
    hero_title: "Where Clinical Leadership Meets Data Science & AI",
    hero_lead:
      "We combine seasoned clinical leadership with data-driven insight to " +
      "support boutique clinical trials with agility and efficiency.",
    hero_cta_primary: "Request a Meeting",
    hero_cta_secondary: "View Services",
    hero_badge1: "Clinical Leadership",
    hero_badge2: "Lean Operations",
    hero_badge3: "AI-Powered Insights",
    panel_title: "Company One-liner",
    panel_text:
      "A boutique clinical services company centered on clinical leadership " +
      "and execution, enhanced by data science and AI.",
    panel_tile1_title: "APAC · EU · US",
    panel_tile1_text: "Experience across global regions.",
    panel_tile2_title: "Rapid Availability",
    panel_tile2_text: "Quick deployment when needed.",
    panel_tile3_title: "Flexible Contracting",
    panel_tile3_text: "Modular, scalable operating model.",
    about_eyebrow: "About Us",
    about_title: "Senior-led, execution-first clinical support",
    about_body:
      "We provide boutique clinical services across the full clinical trial " +
      "lifecycle in diverse environments. Senior clinical leaders guide " +
      "projects directly, leveraging data science and AI tools to clarify " +
      "decisions, optimize timelines, and reduce operational risk.",
    about_card1_title: "What Sets Us Apart",
    about_card1_text:
      "Unlike traditional CRO models, we operate lean teams with direct " +
      "communication to reduce complexity and drive execution.",
    about_card2_title: "Mission & Vision",
    about_card2_text:
      "Our goal is to deliver senior-led clinical services and bridge the " +
      "operational gap with large CROs as a boutique partner.",
    slides_eyebrow: "Slide Highlights",
    slides_title: "Key Message Snapshot",
    slides_body: "Selected highlights from the meeting deck.",
    slide1_title: "Clinisch & Co.",
    slide1_text: "Where Clinical Leadership Meets Data Science & AI",
    slide2_title: "Why Clinisch & Co.",
    slide2_text: "Senior-led model, flexibility, and tech-enabled visibility.",
    slide3_title: "How We Work",
    slide3_text: "Clear communication and fast execution.",
    services_eyebrow: "Our Services",
    services_title: "End-to-end clinical trial support",
    services_body: "Designed flexibly to fit your needs and phase.",
    service1_title: "Clinical Project Management",
    service1_item1:
      "Senior-led clinical project management from planning to delivery",
    service1_item2: "Trial planning, scheduling, and milestone management",
    service1_item3: "Internal team and vendor coordination",
    service1_item4: "Proactive risk identification and issue resolution",
    service1_outcome:
      "Expected outcome: clear accountability · predictable timelines " +
      "· aligned stakeholders",
    service1_icon: "Clinical project management icon",
    service2_title: "Start-Up & Trial Execution",
    service2_item1: "Regulatory and IRB/EC submission support",
    service2_item2: "Start-up and readiness support",
    service2_item3: "Trial execution and operations support",
    service2_item4: "Site-focused coordination",
    service2_outcome:
      "Expected outcome: faster start-up · stable operations · reduced risk",
    service2_icon: "Trial execution icon",
    service3_title: "Study Rescue & Operational Oversight",
    service3_item1: "Recovery of delayed or at-risk trials",
    service3_item2: "Interim clinical and operational leadership",
    service3_item3: "Root-cause analysis and CAPA planning",
    service3_item4: "Ongoing operational management and quality improvement",
    service3_outcome:
      "Expected outcome: regained control · normalized timelines " +
      "· restored confidence",
    service3_icon: "Study rescue icon",
    service4_title: "Clinical Systems & Data Enablement",
    service4_item1: "Clinical system mapping and optimization",
    service4_item2: "Data-driven operational dashboards",
    service4_item3: "AI tools to improve efficiency and quality",
    service4_item4: "Process automation and data integration support",
    service4_outcome:
      "Expected outcome: improved visibility · faster decisions " +
      "· operational efficiency",
    service4_icon: "Clinical systems and data enablement icon",
    approach_eyebrow: "Our Approach",
    approach_title: "Technology strengthens decisions, experience sets direction.",
    approach_card1_title: "Clinical Leadership",
    approach_card1_text: "Senior leaders drive strategy and execution directly.",
    approach_card2_title: "Data-driven Operations",
    approach_card2_text: "Operational insights delivered quickly and clearly.",
    approach_card3_title: "AI-enabled Execution",
    approach_card3_text: "AI tools enhance efficiency and risk control.",
    work_eyebrow: "How We Work",
    work_title: "Flexible collaboration for boutique clinical teams",
    work_card1_title: "Engagement Options",
    work_card1_text: "Weekly support · FTE-based support · project packages",
    work_card2_title: "Scalable Support",
    work_card2_text:
      "We scale up or down based on trial phase and needs.",
    clients_eyebrow: "Who We Work With",
    clients_title: "Partners focused on speed and quality of execution",
    client_tag1: "Biotech & medical device startups",
    client_tag2: "Pharma and mid-sized companies",
    client_tag3: "Partners needing in-house clinical operations",
    client_tag4: "Organizations requiring domain clinical leadership",
    why_eyebrow: "Why Clinisch",
    why_title: "Senior-led execution without unnecessary overhead",
    why_card1: "Senior-led boutique clinical services model",
    why_card2: "Operations support focused on execution",
    why_card3: "Data science & AI with global clinical operations experience",
    why_card4: "Agile, lean operating structure",
    why_card5: "Clear communication and fast pace",
    cta_eyebrow: "Call to Action",
    cta_title: "Need flexible clinical support without a large CRO?",
    cta_body:
      "Let’s discuss the right approach for your clinical trial.",
    cta_role: "Yeon-Eui Goo | CEO & Managing Member",
    label_email: "Email",
    label_support: "Support",
    label_tel: "Tel",
    label_mobile: "Mobile",
    cta_location: "Incheon, South Korea",
    footer_text: "Where Clinical Leadership Meets Data Science & AI"
  }
};

const renderHeader = () => {
  const target = document.getElementById("site-header");

  if (!target) {
    return;
  }

  target.innerHTML = `
    <header class="header">
      <div class="logo">
        <img
          src="public/img/Clinisch_Co_wordmark.svg"
          alt="Clinisch &amp; Co."
        />
      </div>
      <nav class="nav">
        <a href="#about" data-i18n="nav_about">소개</a>
        <a href="#services" data-i18n="nav_services">서비스</a>
        <a href="#approach" data-i18n="nav_approach">어프로치</a>
        <a href="#contact" class="contact-toggle">
          <span class="contact-icon" aria-hidden="true">
            <img src="public/img/contact.png" alt="" />
          </span>
          <span class="contact-text" data-i18n="nav_contact">문의하기</span>
        </a>
        <button
          type="button"
          class="lang-toggle"
          data-lang-toggle
          data-i18n="lang_toggle_aria"
          data-i18n-attr="aria-label"
        >
          <span class="lang-icon" aria-hidden="true">
            <img src="public/img/language.png" alt="" />
          </span>
          <span class="lang-text">EN</span>
        </button>
      </nav>
    </header>
  `;
};

const renderFooter = () => {
  const target = document.getElementById("site-footer");

  if (!target) {
    return;
  }

  target.innerHTML = `
    <footer class="footer">
      <p data-i18n="footer_text">임상 리더십과 데이터 사이언스 &amp; AI가 만나는 곳</p>
    </footer>
  `;
};

const blockUserActions = () => {
  const blockedEvents = [
    "copy",
    "cut",
    "paste",
    "contextmenu",
    "dragstart",
    "selectstart"
  ];

  blockedEvents.forEach((eventName) => {
    document.addEventListener(eventName, (event) => {
      event.preventDefault();
    });
  });

  document.querySelectorAll("img").forEach((image) => {
    image.setAttribute("draggable", "false");
  });
};

const getHeaderOffset = () => {
  const header = document.querySelector(".site-header");

  if (!header) {
    return 0;
  }

  return header.getBoundingClientRect().height;
};

const setupSmoothScroll = () => {
  document.querySelectorAll('a[href^="#"]').forEach((link) => {
    link.addEventListener("click", (event) => {
      const hash = link.getAttribute("href");

      if (!hash || hash === "#") {
        return;
      }

      const target = document.querySelector(hash);

      if (!target) {
        return;
      }

      event.preventDefault();

      const headerOffset = getHeaderOffset();
      const targetOffset = target.getBoundingClientRect().top + window.pageYOffset;
      const finalOffset = Math.max(targetOffset - headerOffset - 12, 0);

      window.scrollTo({
        top: finalOffset,
        behavior: "smooth"
      });
    });
  });
};

const updateToggleLabel = (lang) => {
  const toggle = document.querySelector("[data-lang-toggle]");

  if (!toggle) {
    return;
  }

  const label = lang === "ko" ? "EN" : "KO";
  const text = toggle.querySelector(".lang-text");

  if (text) {
    text.textContent = label;
    return;
  }

  toggle.textContent = label;
};

const applyLanguage = (lang) => {
  const current = translations[lang] || translations.ko;

  document.documentElement.lang = lang;

  document.querySelectorAll("[data-i18n]").forEach((element) => {
    const key = element.dataset.i18n;
    const value = current[key];

    if (!value) {
      return;
    }

    const attr = element.dataset.i18nAttr;

    if (attr) {
      element.setAttribute(attr, value);
      return;
    }

    element.textContent = value;
  });

  updateToggleLabel(lang);
  localStorage.setItem("siteLang", lang);
};

const setupLanguageToggle = () => {
  const toggle = document.querySelector("[data-lang-toggle]");

  if (!toggle) {
    return;
  }

  toggle.addEventListener("click", () => {
    const currentLang = document.documentElement.lang || "ko";
    const nextLang = currentLang === "ko" ? "en" : "ko";
    applyLanguage(nextLang);
  });
};

const getInitialLanguage = () => {
  const saved = localStorage.getItem("siteLang");

  if (saved && translations[saved]) {
    return saved;
  }

  return "ko";
};

document.addEventListener("DOMContentLoaded", () => {
  renderHeader();
  renderFooter();
  blockUserActions();
  setupSmoothScroll();

  const initialLanguage = getInitialLanguage();
  applyLanguage(initialLanguage);
  setupLanguageToggle();
});
